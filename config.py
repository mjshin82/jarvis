...
MEET_GLADIA_ENDPOINTING = float(os.getenv("MEET_GLADIA_ENDPOINTING", "0.25"))  # 침묵 N초→발화종료(Gladia 기본 0.05, ↑=덜 끊김)
MEET_GLADIA_MAX_DURATION = float(os.getenv("MEET_GLADIA_MAX_DURATION", "5"))    # 침묵 없어도 N초면 강제 종료
...
AEC = os.getenv("AEC", "off").lower()               # auto | on | off (off: Swift 데몬 미사용, sounddevice 백엔드)
...
# 회의 STT (RealtimeSTT). realtime=실시간 미리보기(자주 도는 경량), model=발화 확정(정확).
# 기본 tiny+small. tiny/base/small/medium/large-v3/large-v3-turbo 등 faster-whisper 모델명.
MEET_STT_REALTIME_MODEL = os.getenv("MEET_STT_REALTIME_MODEL", "tiny")
MEET_STT_MODEL = os.getenv("MEET_STT_MODEL", "small")
