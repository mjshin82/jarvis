...
_stop = None          # asyncio.Event — close() 시 set 되어 재연결 루프 종료
...
async def _connect_once() -> None:
    """한 번 연결해 끊길 때까지 유지. 끊기면 예외를 올려 reconnect_loop 가 재시도하게 한다."""
    global _client, _ready
    try:
        async with _make_client() as client:
            _client, _ready = client, True
            print(f"[iot] MQTT 연결됨: {config.MQTT_HOST}:{config.MQTT_PORT}")
            async for _ in client.messages:   # publish 전용 — 연결만 살려둔다
                pass
    finally:
...
async def connect() -> None:
    ...
    _task = asyncio.create_task(
        ws_backoff.reconnect_loop(_connect_once, _stop, print, label="iot"))
...
async def close() -> None:
    global _task, _ready, _client, _stop
    ...
    if _task is not None:
        ...
        try:
            ...
            pass
    _task, _ready, _client, _stop = None, False, None, None
