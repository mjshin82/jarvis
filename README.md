...
| `simulation.py` / `scenarios/*.md` | 시뮬레이션(롤플레이) 모드 매니저 + 시나리오(페르소나) |
| `qa.py` / `scenarios/*.qa.md` | 시나리오 Q&A 뱅크 — 예상 질문·답변 쌍을 코드가 선택해 사용 |
| `coach.py` | 연습 모드의 LLM 마이크로 작업(평가, live wording 변형, 짧은 반응) |
...
모드 트리거 없이 "영어 연습하자"처럼 모호하게 진입하면, 한국어로 3가지 모드를 안내하고 다음 발화에서 모드를 정한다. 그 단계에서 "그만"이라고 하면 깨끗하게 빠져나옴.

**3가지 모드 모두 Python 상태머신이 흐름을 통제**하고 질문은 `scenarios/<key>.qa.md` 의 Q&A 뱅크에서 코드가 골라 그대로 출력한다. LLM 은 사용자 답에 대한 한 줄 평가(guided/random) / live 의 wording 변형·짧은 반응만 만든다. 4B 로컬 모델에서도 시나리오 충실도가 깨지지 않는다.

- **guided**: 섹션마다 1개씩 라운드로빈으로 뽑아 다양성 확보. 영어 질문 + 정확한 예시 답변 표시.
- **random**: 무작위 1개씩. 예시 답변은 안 보여줌. 답 → 짧은 평가 → 자동 다음.
- **live**: 같은 라운드로빈 순서, 페르소나 톤으로 영어 wording 변형해서 질문. 사용자 답엔 짧은 영어 반응만(캐릭터 유지).
